...
|
*/
// Never mattered for the S3 API (pure REST/XML, no browser redirects) so
// this was left CI's auto-detect default — which resolves to the container's
// internal SERVER_ADDR behind php-fpm/nginx, not a browser-reachable host.
// The admin panel (docs/plans_v2.md) is the first thing here to actually
// call redirect()/site_url(), so it needs a real value: reuse PUBLIC_BASE_URL
// (same var s3.php's presign already trusts as the externally-reachable URL).
$config['base_url'] = (getenv('PUBLIC_BASE_URL') ?: '') . '/';
